...
class CamVidData(object):
    ...
    def _get_train_data(self, images_path, annots_path, shuffle=True):
        ...
        images, labels = self._reader_train_data(queue)
        images = tf.cast(images, tf.float32)

        capacity = int(0.4 * 367) + 3 * self.batch_size
        if shuffle:
            images, labels = tf.train.shuffle_batch([images, labels], batch_size=self.batch_size, num_threads=1,
                                                    capacity=capacity, min_after_dequeue=int(0.4 * 367))
        else:
            images, labels = tf.train.batch([images, labels], batch_size=self.batch_size, num_threads=1,
                                            capacity=capacity)
        return images, labels

    def _reader_train_data(self, queue):
        # file names
        image_name = queue[0]
        label_name = queue[1]
...
class SegNet(object):
    ...
    def __init__(self, data, learning_rate, filter_size):
        ...
        with self.graph.as_default():
            ...
            # output
            self.logits = self.seg_net(filter_size=self.filter_size)
    ...
    def seg_net2(self, filter_size):
        ...
        conv1 = self.conv_layer_with_bias_one(normal, [7, 7, self.image_channel, filter_size], name="conv1")
        pool1, pool1_indices = self.conv_pool_layer_with_bias(conv1, [7, 7, filter_size, filter_size], count=1, name="pool1")
        pool2, pool2_indices = self.conv_pool_layer_with_bias(pool1, [7, 7, filter_size, filter_size], count=2, name="pool2")
        pool3, pool3_indices = self.conv_pool_layer_with_bias(pool2, [7, 7, filter_size, filter_size], count=3, name="pool3")
        pool4, pool4_indices = self.conv_pool_layer_with_bias(pool3, [7, 7, filter_size, filter_size], count=3, name="pool4")
        pool5, pool5_indices = self.conv_pool_layer_with_bias(pool4, [7, 7, filter_size, filter_size], count=3, name="pool5")
...
if __name__ == "__main__":
    cam_vid_data = CamVidData(class_number=32, batch_size=5, data_root_path="CamVid")
    seg_net = SegNet(cam_vid_data, learning_rate=1e-4, filter_size=16)
    runner = Runner(data=cam_vid_data, net=seg_net, model_path="model", result_path=Tools.new_dir("result"))
    runner.train(epochs=1000, loss_freq=5, model_freq=50, test_freq=50)
    runner.test("test")
